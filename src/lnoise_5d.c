...
double
LNoise5D(double x, double y, double z, double s, double t, guint16* shuffle_table)
{
  double xif = NAN, yif = NAN, zif = NAN, sif = NAN, tif = NAN;
  int xi = 0, yi = 0, zi = 0, si = 0, ti = 0;
  double xf = NAN, yf = NAN, zf = NAN, sf = NAN, tf = NAN;
  double cyf = NAN, cxf = NAN, czf = NAN, csf = NAN, ctf = NAN;
  double dp[32] = {NAN};
  double vx = NAN, vy = NAN, vz = NAN, vs = NAN, vt = NAN;
  int h = 0;
  int i = 0;
  int x_idx = 0, y_idx = 0, z_idx = 0, s_idx = 0, t_idx = 0;
  double tmp = NAN;
  ...
  for (i = 0; i < 32; i++)
  { /* vertices = 2^dimension, edges = 2^(dimension-1) * dimension */
    x_idx = i & 1;
    y_idx = (i & 2) >> 1;
    z_idx = (i & 4) >> 2;
    s_idx = (i & 8) >> 3;
    ...
    {
      case 0:
        dp[i] = ((h & 1) ? vx : -vx) + ((h & 2) ? vy : -vy) + ((h & 4) ? vz : -vz) + ((h & 8) ? vt : -vt);
        break;
      case 1:
        dp[i] = ((h & 1) ? vx : -vx) + ((h & 2) ? vy : -vy) + ((h & 4) ? vs : -vs) + ((h & 8) ? vt : -vt);
        break;
      case 2:
        dp[i] = ((h & 1) ? vx : -vx) + ((h & 2) ? vs : -vs) + ((h & 4) ? vz : -vz) + ((h & 8) ? vt : -vt);
        break;
      case 3:
        dp[i] = ((h & 1) ? vs : -vs) + ((h & 2) ? vy : -vy) + ((h & 4) ? vz : -vz) + ((h & 8) ? vt : -vt);
        break;

      case 4:
        dp[i] = ((h & 1) ? vx : -vx) + ((h & 2) ? vy : -vy) + ((h & 4) ? vz : -vz) + ((h & 8) ? vs : -vs);
        break;
    }
  }
